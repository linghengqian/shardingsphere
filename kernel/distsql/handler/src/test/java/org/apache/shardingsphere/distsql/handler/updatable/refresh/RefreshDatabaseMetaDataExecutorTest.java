...
class RefreshDatabaseMetaDataExecutorTest {
    ...
    void assertExecuteWithDatabaseName() {
        ...
        executor.executeUpdate(new RefreshDatabaseMetaDataStatement("foo_db"), contextManager);
        verify(contextManager).reloadDatabase(database);
    }
    
    @Test
    void assertExecuteWithAllDatabases() {
        ContextManager contextManager = mock(ContextManager.class, RETURNS_DEEP_STUBS);
        ShardingSphereDatabase database = mock(ShardingSphereDatabase.class, RETURNS_DEEP_STUBS);
        ...
        executor.executeUpdate(new RefreshDatabaseMetaDataStatement(null), contextManager);
        verify(contextManager).reloadDatabase(database);
    }
    
    @Test
    void assertExecuteWithSystemDatabaseOnly() {
        ContextManager contextManager = mock(ContextManager.class, RETURNS_DEEP_STUBS);
        ShardingSphereDatabase systemDatabase = mock(ShardingSphereDatabase.class, RETURNS_DEEP_STUBS);
        when(systemDatabase.getName()).thenReturn("information_schema");
        when(contextManager.getMetaDataContexts().getMetaData().getAllDatabases()).thenReturn(Collections.singleton(systemDatabase));
        executor.executeUpdate(new RefreshDatabaseMetaDataStatement(null), contextManager);
    }
}
